...
public:
    ...
    bool WriteZBIBESeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBIBESeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBIBESeed_deprecated(uint256& seed);
    bool EraseZBIBESeed();
    bool EraseZBIBESeed_deprecated();

    bool WriteZBIBECount(const uint32_t& nCount);
    bool ReadZBIBECount(uint32_t& nCount);
