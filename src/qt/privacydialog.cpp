                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBIBE ought to be enough for anybody." - Bill Gates, 2017
    ui->zBIBEpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBIBESyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBIBEpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBIBE_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBIBE is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBIBE...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBIBE in ") +
...
void PrivacyDialog::on_pushButtonSpendzBIBE_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBIBE is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBIBE
        sendzBIBE();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBIBE
    sendzBIBE();
}

void PrivacyDialog::on_pushButtonZBIBEControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ZBIBEControlDialog* zBIBEControl = new ZBIBEControlDialog(this);
    zBIBEControl->setModel(walletModel);
    zBIBEControl->exec();
}

void PrivacyDialog::setZBIBEControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBIBESelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBIBE()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid BibeCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBIBEpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBIBE is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BIBE </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBIBE</b>";
    ...
    // use mints from zBIBE selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBIBEControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZBIBEControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zBIBE require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zBIBE"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZBIBE_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zBIBE require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zBIBE"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zBIBE was spent successfully update the addressbook with the label
    ...
    // Clear zBIBE selector in case it was used
    ZBIBEControlDialog::setSelectedMints.clear();
    ui->labelzBIBESelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBIBE Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " BIBE, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBIBE Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zbibeTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBIBE </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBIBE "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBIBE "));
    ui->labelzBIBEAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBIBESyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzBIBE->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zBIBE
        ui->pushButtonMintzBIBE->setEnabled(false);
        ui->pushButtonMintzBIBE->setToolTip(tr("zBIBE is currently disabled due to maintenance."));

        // Spend zBIBE
        ui->pushButtonSpendzBIBE->setEnabled(false);
        ui->pushButtonSpendzBIBE->setToolTip(tr("zBIBE is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zBIBE
        ui->pushButtonMintzBIBE->setEnabled(true);
        ui->pushButtonMintzBIBE->setToolTip(tr("PrivacyDialog", "Enter an amount of BIBE to convert to zBIBE", 0));

        // Spend zBIBE
        ui->pushButtonSpendzBIBE->setEnabled(true);
        ui->pushButtonSpendzBIBE->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
