...
#include "ui_zbibecontroldialog.h"
...
std::set<std::string> ZBIBEControlDialog::setSelectedMints;
std::set<CMintMeta> ZBIBEControlDialog::setMints;

ZBIBEControlDialog::ZBIBEControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBIBEControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZBIBEControlDialog::~ZBIBEControlDialog()
{
    delete ui;
}

void ZBIBEControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBIBEControlDialog::updateList()
{
...
void ZBIBEControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBIBEControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZBIBE_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBIBEControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZBIBEControlDialog::GetSelectedMints()
{
...
void ZBIBEControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
