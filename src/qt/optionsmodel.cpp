...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BIBE);
    ...
    if (!settings.contains("nAnonymizeBibecoinAmount"))
        settings.setValue("nAnonymizeBibecoinAmount", 1000);

    nAnonymizeBibecoinAmount = settings.value("nAnonymizeBibecoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBibecoinAmount"))
        SoftSetArg("-anonymizebibecoinamount", settings.value("nAnonymizeBibecoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bibecoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBibecoinAmount:
            return QVariant(nAnonymizeBibecoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBibecoinAmount:
            nAnonymizeBibecoinAmount = value.toInt();
            settings.setValue("nAnonymizeBibecoinAmount", nAnonymizeBibecoinAmount);
            emit anonymizeBibecoinAmountChanged(nAnonymizeBibecoinAmount);
            break;
        case CoinControlFeatures:
