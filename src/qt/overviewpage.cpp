...
// Copyright (c) 2017-2018 The BibeCoin developers
...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::BIBE)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sBIBEPercentage, QString& szBIBEPercentage)
{
    int nPrecision = 2;
    ...
    szBIBEPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sBIBEPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount bibeAvailableBalance = balance - immatureBalance - nLockedBalance;
    ...
    // Combined balances
    CAmount availableTotalBalance = bibeAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // BIBE labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnitComma(nDisplayUnit, bibeAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelBIBEPercent->setText(sPercentage);
    ui->labelzBIBEPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zBIBE.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in bibecoin.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in bibecoin.conf");
    }
    ...
    bool showBIBEAvailable = settingShowAllBalances || bibeAvailableBalance != nTotalBalance;
    bool showWatchOnlyBIBEAvailable0 = watchOnlyBalance != nTotalWatchBalance;
    bool showBIBEPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyBIBEPending = watchUnconfBalance != 0;
    bool showBIBELocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyBIBELocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showBIBEAvailable || showWatchOnlyBIBEAvailable0);
    ui->labelBalanceText->setVisible(showBIBEAvailable || showWatchOnlyBIBEAvailable0);
    ui->labelWatchAvailable->setVisible(showBIBEAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showBIBEPending || showWatchOnlyBIBEPending);
    ui->labelPendingText->setVisible(showBIBEPending || showWatchOnlyBIBEPending);
    ui->labelWatchPending->setVisible(showBIBEPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showBIBELocked || showWatchOnlyBIBELocked);
    ui->labelLockedBalanceText->setVisible(showBIBELocked || showWatchOnlyBIBELocked);
    ui->labelWatchLocked->setVisible(showBIBELocked && showWatchOnly);
    ...
    bool showzBIBEAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzBIBEUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzBIBEImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzBIBEAvailable);
    ui->labelzBalanceMatureText->setVisible(showzBIBEAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzBIBEUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzBIBEUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzBIBEImmature);
    ui->labelzBalanceImmatureText->setVisible(showzBIBEImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelBIBEPercent->setVisible(showPercentages);
    ui->labelzBIBEPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("BIBE")
    updateDisplayUnit();
}
