...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zbibeTracker = unique_ptr<CzBIBETracker>(new CzBIBETracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZBIBE_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZBIBE_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
