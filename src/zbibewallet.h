...
#ifndef BIBECOIN_ZBIBEWALLET_H
#define BIBECOIN_ZBIBEWALLET_H
...
public:
    CzBIBEWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZBIBE(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZBIBE(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //BIBECOIN_ZBIBEWALLET_H
