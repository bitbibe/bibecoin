...
CzBIBETracker::CzBIBETracker(std::string strWalletFile)
{
...
CzBIBETracker::~CzBIBETracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBIBETracker::Init()
{
...
bool CzBIBETracker::Archive(CMintMeta& meta)
{
...
bool CzBIBETracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBIBETracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBIBETracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBIBETracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzBIBETracker::GetSerialHashes()
{
...
CAmount CzBIBETracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBIBETracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBIBETracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBIBETracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBIBETracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBIBETracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBIBETracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBIBETracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBIBETracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBIBETracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBIBETracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBIBETracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBIBETracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBIBETracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBIBETracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBIBETracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBIBETracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzbibe from DB\n", __func__, listDeterministicDB.size());
    }
