...
CzBIBEWallet::CzBIBEWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zBIBE seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBIBESeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBIBESeed_deprecated()) {
                    LogPrintf("%s: Updated zBIBE seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zBIBE seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zBIBE wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBIBEWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBIBECount(nCountLastUsed);
    else if (!walletdb.ReadZBIBECount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBIBEWallet::Lock()
{
    seedMaster = 0;
}

void CzBIBEWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBIBEWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBIBE(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBIBEWallet::LoadMintPoolFromDB()
{
...
void CzBIBEWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBIBEWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBIBEWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbibeTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBIBEWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBIBE(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbibeTracker which also adds to database
    pwalletMain->zbibeTracker->Add(dMint, true);
...
void CzBIBEWallet::SeedToZBIBE(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBIBEWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBIBEWallet::GenerateDeterministicZBIBE(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBIBEWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBIBEWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
