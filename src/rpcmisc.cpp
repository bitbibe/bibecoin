...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total bibecoin balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zBIBEsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zBIBE denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zBIBE denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zBIBE denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zBIBE denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zBIBE denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zBIBE denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zBIBE denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zBIBE denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zBIBE denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in bibecoin/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in bibecoin/kb\n"
    ...
    UniValue zbibeObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zbibeObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zbibeObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("zBIBEsupply", zbibeObj));
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are bibecoin addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) bibecoin address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"bibecoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"bibecoinaddress\"  (string, required) The bibecoin address to use for the signature.\n"
