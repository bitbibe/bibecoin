...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 20000 BIBE at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBIBE send
enum ZerocoinSpendStatus {
    ZBIBE_SPEND_OKAY = 0,                            // No error
    ZBIBE_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBIBE_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBIBE_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBIBE_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBIBE_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBIBE_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBIBE_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBIBE_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBIBE_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBIBE_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBIBE_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBIBE_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBIBE_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBIBE_SPENT_USED_ZBIBE = 14,                      // Coin has already been spend
    ZBIBE_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZBIBE_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    void ZBIBEBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZBIBEOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzbibeAuto) const;
    ...
    */
    ...
    CzBIBEWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzBIBETracker> zbibeTracker;
    ...
    void setZWallet(CzBIBEWallet* zwallet)
    {
        zwalletMain = zwallet;
        zbibeTracker = std::unique_ptr<CzBIBETracker>(new CzBIBETracker(strWalletFile));
    }

    CzBIBEWallet* getZWallet() { return zwalletMain; }
    ...
    void setZBIBEAutoBackups(bool fEnabled)
    {
